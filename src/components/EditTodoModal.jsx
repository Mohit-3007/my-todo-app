...
const EditTodoModal = ({todo, setIsEdit}) => {
    ...
    return (
      <div className='w-screen h-screen fixed top-0 left-0 bg-black bg-opacity-70 flex justify-center items-center'>
        <div ref={taskModalRef} className='w-[31.25rem] h-[26.75rem] bg-[#354259] rounded-md border-2 border-white 
          py-5 text-center relative'>
          {/* heading */}
          <h3 className='text-xl tracking-wider text-white font-bold'>EDIT TASK</h3>
          {/* Task Input Field */}
          <input type='text'
            placeholder='Type your task here' 
            className='w-[26.25rem] h-[2.375rem] px-4 rounded-md outline-none text-lg font-semibold mt-5 mb-[2.0806rem]' 
            value={taskInput}
          ...
          <div className='w-full h-[11.1762rem] px-1'>
            ...
            {/* Category Button */}
            <div className='w-full mt-[1.875rem] px-11 grid grid-cols-2 gap-5'>
              {categoryList && categoryList.map((list, idx) => {
                return (
                  <div key={idx} className={`w-[11.875rem] h-[1.7813rem] rounded-md  duration-300 ease-linear ` +
